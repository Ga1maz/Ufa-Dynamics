...
void controlLEDs() {
    ...
    if (yellowBlinking) {
        ...
        // Синие светодиоды для режима настройки
        if (millis() - lastBlinkTime > 500) {
            ledState = !ledState; // Переключить состояние LED
            if (ledState) {
                fill_solid(leds, LED_COUNT, CRGB::Blue); // Установить светодиоды в синий цвет
            } else {
                fill_solid(leds, LED_COUNT, CRGB::Black); // Выключить светодиоды
            }
            FastLED.show();
        ...
        digitalWrite(CONTROL_PIN, LOW); // Убедитесь, что пин 19 выключен в этом режиме
        ...
        // Зеленые светодиоды при нормальной работе
        fill_solid(leds, LED_COUNT, CRGB::Green); // Установить светодиоды в зеленый цвет
        FastLED.show();
        digitalWrite(CONTROL_PIN, LOW); // Убедитесь, что пин 19 выключен в этом режиме
    }
